...
テナントでプロビジョニングされた学習経路を持たない組織では、SharePoint プロビジョニングサービスを使用して多言語の学習経路ソリューションを追加することができます。 このオプションを使用すると、ラーニングパス SharePoint テンプレートが9つの言語に翻訳され、最小限の変更で使用できます。 

> [!IMPORTANT]
> テナントに既に学習経路がプロビジョニングされている場合は、ラーニング経路の[更新パス](custom_update_ml.md)に従うことをお勧めします。 テナント内の既存のインスタンスに学習経路をインストールした場合、学習機能のサイトテンプレートまたはプレイリストに加えられた変更が失われることがあります。
...
プロビジョニングサービスには、テナントのアプリカタログを作成し、アプリケーションをテナントのアプリカタログにインストールし、サイトテンプレートをプロビジョニングするためのアクセス許可が必要です。 テナントに全体的な影響はありません。 これらのアクセス許可は、ソリューションのインストールの目的で明示的に使用されます。 インストールを続行するには、これらのアクセス許可を受け入れる必要があります。
...
SharePoint プロビジョニングサービスは、学習経路サイトを9つの言語で作成します。 次の推奨事項が適用されます。
- サポートしたくない言語をオフにする
